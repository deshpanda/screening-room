...
export function centuryStrip(years, endYear) {
  ...
  for (const [a, b, label] of ERAS) {
    ...
    if (w <= 0) continue;
    // era ribbon lives in the footer, under the decade ticks
    svg.appendChild(el('rect', { class: `era era-${band % 2}`, x, y: base + 22, width: w, height: 30 }));
    svg.appendChild(el('text', {
      class: 'axis era-label', x: x + 5, y: base + (band % 2 ? 44 : 33),
    }, label));
    band++;
  }
