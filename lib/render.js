...
const ERAS = [
  ...
  [1981, 1989, 'high concept'], [1990, 1999, 'the independents'], [2000, 2009, 'world cinema'],
  [2010, 2019, 'streaming'], [2020, 9999, 'now'],
];
export function centuryStrip(years, endYear) {
  const byYear = new Map(years.map((r) => [r.y, r]));
  const first = 1895;
  const colW = 9, dotR = 2.6, dotGap = 7, padB = 44, padT = 12;
  ...
  for (const [a, b, label] of ERAS) {
    ...
    if (w <= 0) continue;
    svg.appendChild(el('rect', { class: `era era-${band % 2}`, x, y: 0, width: w, height: H - padB + 8 }));
    svg.appendChild(el('text', {
      class: 'axis era-label', x: x + 4, y: H - (band % 2 ? 6 : 20),
    }, label));
    band++;
  }
  for (let y = first; y <= endYear; y++) {
    ...
    if (y % 10 === 0) {
      svg.appendChild(el('text', { class: 'axis', x, y: H - padB + 4, 'text-anchor': 'middle' }, `’${String(y).slice(2)}`));
    }
    ...
    for (let i = 0; i < shown; i++) {
      svg.appendChild(el('circle', {
        class: 'century-dot', cx: x, cy: H - padB - 10 - i * dotGap, r: dotR,
      }));
    }
    const hit = el('rect', { x: x - colW / 2, y: 0, width: colW, height: H - padB, fill: 'transparent' });
